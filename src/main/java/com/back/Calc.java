...
public class Calc {
    ...
    public static int run(String s) {
        ...
        return parsP(exp);  // 괄호 분리 및 처리 > 덧셈 양쪽 분리 > 곱셈 처리 > 덧셈 처리
    }
    // 괄호 처리
    private static int parsP(String exp) {
        ...
        if (lpidx != -1){ // 괄호 있음
            ...
            return parsP(exp.substring(0, lpidx)+parsP(exp.substring(lpidx+1, rpidx))+exp.substring(rpidx+1));
        } else { // 괄호 없음
            return calc(exp);
        }
    }
    // 덧셈 양쪽 분리 > (곱셈 처리) > 덧셈 처리
    ...
    // 덧셈 처리
    private static int add(String exp) {
        return Arrays.stream(exp.trim().split(" \\+ ")).mapToInt(Integer::parseInt).sum();
    }
    // 곱셈 처리
    private static int mult(String exp) {
